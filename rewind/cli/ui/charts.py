"""
Chart Utilities for GPT Rewind CLI
"""
...
def _print_legend(items):
    ...
    for item in items:
        ...
        legend_item = (
            f"[{item['color']}]■[/{item['color']}] {item['label']}:"
            f"            [bold]{val_display}[/bold] ({pct_str})"
        )
...
def print_distribution_bar(data, title=None, label_key="key", value_key="value"):
    ...
    for i, item in enumerate(items):
        ...
        # For the last item, take the remaining space
        # to ensure full width (if it's significant)
        if i == len(items) - 1:
        ...
        if seg_len > 0:
            ...
            # If segment is too small to show a char,
            # we might skip it in the bar but show in legend
            pass
